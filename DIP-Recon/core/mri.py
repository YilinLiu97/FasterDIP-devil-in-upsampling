...
def main(args):
    ...
    assert args.task == 'mri_knee' or args.task == 'mri_brain'
    ...
    for iter, sample in enumerate(train_data):
       ...
       _, trained_net, psnr_score, ssim_score, spectral_norms, singular_list = fit(args,
                          net,
       ...
       # Analysis
       info = {'noisy_target': undersampled_recon, 'rec': rec, 'orig': orig, 'p_score': metrics['psnr'], 's_score': metrics['ssim'],'noisy_p_score': metrics_noisy['psnr'],
               'noisy_s_score': metrics_noisy['ssim'],'filename': filename_full, 'savename': (args.exp_name + '/results')}
       visualize(info)
       save_scores(args, info)

       if rec2_available:
          info2 = {'noisy_target': undersampled_recon, 'rec': rec2, 'orig': orig, 'p_score': metrics2['psnr'], 's_score': metrics2['ssim'],
                    'filename': filename_full, 'savename': (args.exp_name + '/pruning_results')}
          visualize(info2)
          save_scores(args, info2)

       plot_metrics(f"{args.exp_name}/{args.num_iters}th_epoch.pt", f"{args.exp_name}/metric_plots")

       if spectral_norms is not None:
          plot_multiple_spectral_norms(spectral_norms, f"{args.exp_name}/spectral_norm_all_layers")
          plot_singular_values_all_iters(singular_list, f"{args.exp_name}/singular_values")
