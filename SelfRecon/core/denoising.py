...
def main(args):
    ...
    assert args.task == 'denoising' or args.task == 'sr' or args.task == 'real_denoising'
    ...
    for iter, sample in enumerate(train_data):
        ...
        out_avg, trained_net, psnr_score, ssim_score, spectral_norms, singular_list, noisy_psnr, noisy_ssim = fit(args,
                                                  net,
...
#            print(compare_psnr(torch_to_np(orig).transpose(1,2,0), torch_to_np(rec).transpose(1,2,0))
        ...
        if layerID is not None:
           info = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': rec, 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics['psnr'], 's_score': metrics['ssim'], 'noisy_p_score': noisy_metrics['psnr'],
               'noisy_s_score': noisy_metrics['ssim'], 'filename': filename_full, 'savename': (f"{args.exp_name}/results"), 'depth': layerID}
        else:
           info = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': rec, 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics['psnr'], 's_score': metrics['ssim'], 'noisy_p_score': noisy_metrics['psnr'],
               'noisy_s_score': noisy_metrics['ssim'], 'filename': filename_full, 'savename': (f"{args.exp_name}/results"), 'baseline': noisy_psnr}

        visualize(info)
        save_scores(args, info)

        if rec2_available:
           info2 = {'noisy_target': noisy_target[0].data.cpu().numpy(), 'rec': rec2, 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics2['psnr'], 's_score': metrics2['ssim'], 'noisy_p_score': noisy_metrics['psnr'],
...
if __name__ == '__main__':
    ...
    with open(args.csv_filename, 'wt') as csv_file:
       ...
       headers = ['ID', 'PSNR', 'SSIM', 'Noisy_Baseline']
