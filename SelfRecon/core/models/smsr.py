...
class SMSR(nn.Module):
    def __init__(self, n_feats=64, scale=2, n_colors=3, rgb_range=255, conv=smsr_helper.default_conv):
        super(SMSR, self).__init__()

        n_feats = n_feats
        kernel_size = 3
        self.scale = scale #int(args.scale[0])

        # RGB mean for DIV2K
        rgb_mean = (0.4488, 0.4371, 0.4040)
        rgb_std = (1.0, 1.0, 1.0)
        self.sub_mean = smsr_helper.MeanShift(rgb_range, rgb_mean, rgb_std)
        ...
        self.add_mean = smsr_helper.MeanShift(rgb_range, rgb_mean, rgb_std, 1)
