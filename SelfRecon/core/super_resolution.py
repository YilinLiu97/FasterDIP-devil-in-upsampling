...
def main(args):
    ...
    for iter, sample in enumerate(train_data):
        ...
        bicubic_img = sample['bicubic_img']
        nearest_img = sample['nearest_img']
        ...
        bicubic_metrics = eval_general(torch_to_np(orig), torch_to_np(bicubic_img))
        nearest_metrics = eval_general(torch_to_np(orig), torch_to_np(nearest_img))
        ...
        # Analysis
        info = {'noisy_target': nearest_img[0].data.cpu().numpy(), 'rec': rec, 'orig': orig[0].data.cpu().numpy(), 'p_score': metrics['psnr'], 's_score': metrics['ssim'], 'noisy_p_score': nearest_metrics['psnr'],
           'noisy_s_score': nearest_metrics['ssim'], 'filename': filename_full, 'savename': (f"{args.exp_name}/results")}
